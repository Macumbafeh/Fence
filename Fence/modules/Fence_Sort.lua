...
L:RegisterTranslations("enUS", function() return {
	["Sort"] = true,
    ["Adds the ability to sort by item name."] = true,
} end)

L:RegisterTranslations("deDE", function() return {
	["Sort"] = "Sort",
    ["Adds the ability to sort by item name."] = "Erm\195\182glicht die Sortierung nach Gegenstandnamen.",
} end)

L:RegisterTranslations("zhTW", function() return {
	["Sort"] = "排序",
    ["Adds the ability to sort by item name."] = "增加以名稱排序功能。",
} end)

L:RegisterTranslations("zhCN", function() return {
	["Sort"] = "排序",
    ["Adds the ability to sort by item name."] = "增加按名称排序功能",
} end)
...
mod.db = Fence:AcquireDBNamespace("Sort")
	...
	function mod:OnDisable()
		if self:IsHooked(BrowseCurrentBidSort, "SetWidth") then
			self:Unhook(BrowseCurrentBidSort, "SetWidth") end
	end
